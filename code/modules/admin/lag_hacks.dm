...
/datum/admins/proc/show_laghacks(mob/user)
	...
	HTML += "<b><a href='?src=\ref[src];action=disable_deletions'>Disable Deletion Queue (Garbage Collection will still run, but this stops hard deletions from happening. Might be a bit dangerous depending on memory stuff, but its usually safe and especially in late round.)</a></b><br>"
	HTML += "</body></html>"
